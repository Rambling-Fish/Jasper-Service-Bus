...
public class DataConsumer implements Runnable {
	...
	private void processRequest() throws Exception {
		...
		if (jsonRequest.isJsonObject() && jsonRequest.getAsJsonObject().has("headers") && jsonRequest.getAsJsonObject().get("headers").isJsonObject()
		...
		if (jsonRequest.isJsonObject() && jsonRequest.getAsJsonObject().has("parameters") && jsonRequest.getAsJsonObject().get("parameters").isJsonObject()) {
...
//			polling = statefulData.getTriggers().get(0).getPolling();
	...
	private void processPublish(String ruri, JsonElement parameters) throws Exception {
		if (parameters.isJsonObject() && parameters.getAsJsonObject().has("parmsArray") && parameters.getAsJsonObject().get("parmsArray").isJsonArray()) {
	...
	private void sendResponse(String response) throws JMSException {
		...
		if (correlationID == null)
			correlationID = statefulData.getKey();
